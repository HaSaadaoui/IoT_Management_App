class DashboardManager {
  constructor() {
    ...
    this.virtualBuildings = {
      CHATEAUDUN: { id: 'CHATEAUDUN', code: 'CHATEAUDUN', name: 'Châteaudun', floors: 6 },
      LEVALLOIS: { id: 'LEVALLOIS', code: 'LEVALLOIS', name: 'Levallois', floors: 1 }
    };
  ...
  async updateLiveBuildingMetrics() {
    ...
    try {
      ...
      const requests = sensorTypes.map(sensorType => {
        const params = new URLSearchParams({ building, floor, sensorType });

        return fetch(`/api/dashboard/sensors?${params}`)
      ...
      results.forEach(r => {
        if (r && r.data) metricsData[r.sensorType] = r.data;
      });
  ...
  calculateAndUpdateMetrics(metricsData) {
    // Temperature
    if (metricsData.TEMP?.length) {
      const temps = metricsData.TEMP.map(s => s.lastValue).filter(v => v !== null && v !== undefined);
      if (temps.length) {
    ...
    // Humidity
    if (metricsData.HUMIDITY?.length) {
      const humidity = metricsData.HUMIDITY.map(s => s.lastValue).filter(v => v !== null && v !== undefined);
      if (humidity.length) {
    ...
    // Noise
    if (metricsData.NOISE?.length) {
      const noise = metricsData.NOISE.map(s => s.lastValue).filter(v => v !== null && v !== undefined);
      if (noise.length) {
    ...
    // CO2
    if (metricsData.CO2?.length) {
      const co2 = metricsData.CO2.map(s => s.lastValue).filter(v => v !== null && v !== undefined);
      if (co2.length) {
    ...
    // Energy
    if (metricsData.ENERGY?.length) {
      const energySensors = metricsData.ENERGY;
      const currentPowers = energySensors.map(s => s.lastValue).filter(v => v !== null && v !== undefined);
    ...
    // Light
    if (metricsData.LIGHT?.length) {
      const lightLevels = metricsData.LIGHT.map(s => s.lastValue).filter(v => v !== null && v !== undefined);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.data && Array.isArray(data.data)) {
          const todayData = data.data.filter(entry => new Date(entry.timestamp) >= startOfDay);
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            ticks: { maxTicksLimit: 8, font: { size: 11 }, maxRotation: 45, minRotation: 22 }
          },
          y: {
            display: true,
            title: { display: true, text: 'Cost (€)', font: { size: 14, weight: 'bold' } },
            grid: { color: 'rgba(0,0,0,0.1)', drawBorder: false },
            ticks: { maxTicksLimit: 6, font: { size: 11 }, callback: v => '€' + Number(v).toFixed(2) }
          }
        },
        plugins: {
    ...
    this.charts.costArea = new Chart(chartElement, {
      ...
      options: {
        ...
        scales: {
          x: {
            display: true,
            title: { display: true, text: 'Date', color: '#64748b', font: { size: 14, weight: '600' } },
            grid: { display: false },
            ticks: { maxRotation: 45, minRotation: 45, font: { size: 11 } }
          },
          y: {
            display: true,
            title: { display: true, text: 'Cost (€)', color: '#64748b', font: { size: 14, weight: '600' } },
            beginAtZero: true,
            ticks: { font: { size: 11 }, callback: v => '€' + Number(v).toFixed(2) },
        ...
        plugins: {
          ...
          tooltip: {
            ...
            borderColor: '#10b981',
            borderWidth: 2,
            callbacks: { label: ctx => 'Cost: €' + ctx.parsed.y.toFixed(2) }
          }
        }
      }
    });
  ...
  renderDeskOccupancyBars(data) {
    ...
    this.charts.histogram = new Chart(chartElement, {
      ...
      options: {
        ...
        scales: {
          x: {
            title: { display: true, text: 'Individual Desks', color: '#64748b', font: { size: 14, weight: '600' } },
            ticks: { maxRotation: 90, minRotation: 45, font: { size: 9 }, autoSkip: false },
            grid: { display: false }
          },
          y: {
            title: { display: true, text: 'Status (0=Free, 1=Occupied)', color: '#64748b', font: { size: 14, weight: '600' } },
            beginAtZero: true,
            ticks: { font: { size: 11 } },
            grid: { color: 'rgba(0, 0, 0, 0.05)' }
          }
        ...
        plugins: {
          legend: {
            display: true,
            position: 'top',
            labels: { font: { size: 12, weight: '600' }, color: '#34495e' }
          },
          tooltip: {
            backgroundColor: 'rgba(0, 0, 0, 0.8)',
            padding: 12,
  ...
  updateHistogramSummary(summary) {
    ...
    // Dummy test data (comme ton code)
