...
// =================================================
//
// Le backend envoie deux types d'events SSE :
//   "snapshot" → données initiales chargées via HTTP TTN (Storage API)
//   "uplink"   → données live reçues via MQTT TTN
//
// Les deux events portent le même format JSON normalisé par SensorEventNormalizer,
// donc handleSensorData() traite les deux de façon identique.
...
function startSSE() {
    ...
    // ─── Handler commun snapshot + uplink ────────────────────────────────────
    // "snapshot" = données initiales HTTP TTN  |  "uplink" = live MQTT TTN
    // Le format JSON est identique dans les deux cas (normalisé côté backend).
          ...
          `;
            ...
            switch (PROF) {
                ...
                case 'OCCUP':
                    if (p.presence != null && el('#s-occup-presence')) {
                        ...
                        if (presenceValue === 'occupied' || presenceValue === '1' || p.presence === 1 || p.presence === true) {
                            occNode.classList.add('badge--occupied');
                ...
                case 'EYE':
                    ...
                    if (p.light != null && el('#s-eye-light')) setText('#s-eye-light', p.light + ' lux');
    ...
    // snapshot = données initiales HTTP TTN (Storage API)
    es.addEventListener("snapshot", handleSensorData);
    // uplink   = données live MQTT TTN
    es.addEventListener("uplink", handleSensorData);
...
const consumptionCharts = {
    'red':   { id: 'histConsumptionAll', channels: [0, 1, 2],  label: 'Red Outlets',             color: 'rgb(239, 68, 68, 1)'  },
    'white': { id: 'histConsumptionAll', channels: [3, 4, 5],  label: 'White Outlets & Lightning', color: 'rgb(100, 116, 139, 1)' },
    'vent':  { id: 'histConsumptionAll', channels: [6, 7, 8],  label: 'Ventilation & Heaters',    color: 'rgb(59, 130, 246, 1)' },
    'other': { id: 'histConsumptionAll', channels: [9, 10, 11], label: 'Other Circuits',           color: 'rgb(245, 158, 11, 1)' }
};
...
const METRIC_TITLES = {
    'CO2': 'CO₂ (ppm)', 'TEMPERATURE': 'Temperature (°C)', 'HUMIDITY': 'Humidity (%)',
    'VDD': 'Voltage (mV)', 'LIGHT': 'Light (lux)', 'MOTION': 'Motion', 'PRESENCE': 'Presence',
    'OCCUPANCY': 'Occupancy', 'PERIOD_IN': 'Number of employees in', 'PERIOD_OUT': 'Number of employees out',
    'LAI': 'Sound Impact (LAI)', 'LAIMAX': 'Max Sound Impact (LAImax)', 'LAEQ': 'Equivalent Sound Level (LAeq)',
    'BATTERY': 'Battery (%)', 'LAST_BATTERY_PERCENTAGE': 'Battery (%)', 'RSSI': 'Signal (RSSI)',
    'SNR': 'Signal/Noise (SNR)', 'DISTANCE': 'Distance (mm)', 'ILLUMINANCE': 'Illuminance',
    'ENERGY_CHANNEL_0': 'Energy Channel 0', 'ENERGY_CHANNEL_1': 'Energy Channel 1',
    'ENERGY_CHANNEL_2': 'Energy Channel 2', 'ENERGY_CHANNEL_3': 'Energy Channel 3',
    'ENERGY_CHANNEL_4': 'Energy Channel 4', 'ENERGY_CHANNEL_5': 'Energy Channel 5',
    'ENERGY_CHANNEL_6': 'Energy Channel 6', 'ENERGY_CHANNEL_7': 'Energy Channel 7',
    'ENERGY_CHANNEL_8': 'Energy Channel 8', 'ENERGY_CHANNEL_9': 'Energy Channel 9',
    'ENERGY_CHANNEL_10': 'Energy Channel 10', 'ENERGY_CHANNEL_11': 'Energy Channel 11',
};

function getMetricColor(metricName) {
    const colors = {
        'temperature': '#f59e0b', 'humidity': '#3b82f6', 'co2': '#ef4444', 'vdd': '#10b981',
        'light': '#fbbf24', 'motion': '#6366f1', 'presence': '#10b981', 'occupancy': '#10b981',
        'period_in': '#6366f1', 'period_out': '#8b5cf6', 'lai': '#ec4899', 'laeq': '#db2777',
        'laimax': '#9333ea', 'battery': '#059669', 'rssi': '#2563eb', 'snr': '#7c3aed',
    };
    return colors[metricName.toLowerCase()] || '#662179';
}
...
async function loadHistory(fromISO, toISO) {
    ...
    const getChartData = (metricName) => {
        ...
        if (dataPoints > GROUPING_THRESHOLD) {
            ...
            for (const timestamp in rawData) {
                ...
                const groupHour = Math.floor(date.getHours() / 6) * 6;
                date.setHours(groupHour, 0, 0, 0);
                const groupKey = date.toISOString();
                if (!groupedData[groupKey]) groupedData[groupKey] = { sum: 0, count: 0 };
                groupedData[groupKey].sum += parseFloat(rawData[timestamp]);
                groupedData[groupKey].count++;
            }
            const averagedData = {};
            for (const key in groupedData) averagedData[key] = groupedData[key].sum / groupedData[key].count;
            return {
                labels: Object.keys(averagedData).map(t => new Date(t).toLocaleString('en-GB', { day: '2-digit', month: '2-digit', hour: '2-digit' })),
                values: Object.values(averagedData)
            };
        }

        const metricData = j.data[metricName] || {};
        return {
            labels: Object.keys(metricData).map(t => new Date(t).toLocaleString()),
            values: Object.values(metricData)
        };
    };
    ...
    if (devType === 'ENERGY' || devType === 'CONSO') {
        ...
        const datasets = allGroupData.map((groupData, index) => {
            ...
            return {
                ...
                data: Object.values(groupData || {}).map(v => v / 1000),
                backgroundColor: groupInfo.color, borderColor: groupInfo.color,
                borderWidth: 1, borderRadius: 4, barPercentage: 0.8,
            };
        });
        if (!combinedConsumptionChart) {
            ...
            if (chartCtx) {
                combinedConsumptionChart = new Chart(chartCtx, {
                    ...
                    options: { responsive: true, maintainAspectRatio: false, scales: { x: { stacked: false }, y: { beginAtZero: true, title: { display: true, text: 'Total Consumption (kWh)' } } } }
                });
            }
        }
        if (combinedConsumptionChart) {
        ...
        allGroupData.forEach((groupData, index) => {
            ...
            if (totalEl) {
                const totalKWh = Object.values(groupsKW).reduce((a, b) => a + b, 0);
    ...
    const processMetric = (metricName, container) => {
        ...
        if (chartCtx) {
            ...
            chartConfig.data = { datasets: [{ data: transformedData, borderColor: color, backgroundColor: bgColor, fill: true, tension: 0.1 }] };

            const generatedLabels = generateLabels(Object.values(j.data[metricName] || {}));
            const yType = containsStrings(generatedLabels) ? 'category' : 'linear';
            chartConfig.options.scales = {
                y: { type: yType, labels: generatedLabels, title: { display: true, text: chartTitle, font: { size: 14, weight: 'bold' } }, beginAtZero: true },
                x: {
                    type: 'time',
                    ...
                    grid: { color: (context) => context.tick.major ? 'rgba(0,0,0,0.3)' : 'rgba(0,0,0,0.05)' }
                }
            };
...
function getBattery(data) {
    ...
    if (lastBattery?.length > 0) return lastBattery.map(x => parseInt(x, 10));
    if (battery?.length > 0) return battery.map(x => parseInt(x, 10));
...
function updateKPICards(data, fromISO, toISO) {
    ...
    if (totalEl) {
        ...
        const total = values.length > 0 ? values.map(x => Object.values(x).length).reduce((a, b) => a + b) : 0;
    ...
    if (batteryEl && pctValues?.length > 0) {
        batteryEl.textContent = `${Math.round(pctValues.reduce((a, b) => a + b, 0) / pctValues.length)}%`;
    }
    ...
    if (rssiEl && rssiValues?.length > 0) {
        rssiEl.textContent = `${Math.round(rssiValues.reduce((a, b) => a + b, 0) / rssiValues.length)} dBm`;
    }
    ...
    if (periodEl && fromISO && toISO) {
        ...
        periodEl.textContent = diffDays > 0 ? `${diffDays} jour${diffDays > 1 ? 's' : ''}` : `${diffHours} heure${diffHours > 1 ? 's' : ''}`;
    }
}
...
function showPane(which) {
    ...
    if (which === 'live') {
        ...
        LIVE_MODE = true;
        // Réinitialise les charts si pas encore créés (canvas était hidden au chargement)
        if (window.Chart && !realtimeCharts.main) {
            initRealtimeCharts();
        }
        startSSE();
    } else {
...
function updateEnergyConsumption(data) {
    const normalized = {};
    Object.keys(data || {}).forEach(k => { normalized[k] = absIfPower(data[k]); });
    ...
    for (let ch = 0; ch <= 11; ch++) {
        ...
        if (channelEl) renderChannelCard(channelEl, ch, channelPowerData[ch]);
    }
    ...
    if (totalEl) {
      ...
      <div class="total-consumption">
        ...
        <div class="total-kwh">${(totalW / 1000).toFixed(3)} kW</div>
    ...
    const channelGroups = {
        ...
        'ventilation':   { channels: [6, 7, 8],  name: 'Ventilation & Heaters',   color: '#3b82f6' },
        'other':         { channels: [9, 10, 11], name: 'Other Circuits',          color: '#f59e0b' }
    };
    const chartDataPower = {};
    for (let ch = 0; ch <= 11; ch++) chartDataPower[ch] = { value: Number(channelPowerData[ch]) || 0 };
    updateEnergyChart(channelGroups, chartDataPower, { mode: 'power', totalsW: { 'red-outlets': redW, 'white-outlets': whiteW, 'ventilation': ventW, 'other': otherW } });
}

function renderChannelCard(channelEl, ch, powerW) {
    if (!channelEl) return;
    const wTxt = (typeof powerW === 'number' && Number.isFinite(powerW)) ? `${Math.round(powerW)} W` : `-- W`;
    channelEl.innerHTML = `<div class="channel-card"><div class="channel-left">Channel ${ch}</div><div class="channel-right">${wTxt}</div></div>`;
}
...
function updateEnergyChart(groups, data, opts = {}) {
    ...
    if (!chartEl) return;
    const mode = opts.mode || 'energy';
    const unit = (mode === 'power') ? 'kW' : 'kWh';
    const totalsW = opts.totalsW || null;

    const groupData = Object.entries(groups).map(([groupId, group]) => {
        let total = (totalsW && typeof totalsW[groupId] === 'number') ? totalsW[groupId] : 0;
        if (!totalsW) group.channels.forEach(ch => { const d = data[ch]; if (d && typeof d.value === 'number') total += d.value; });
        return { name: group.name, value: total / 1000, color: group.color };
    });
    ...
    if (!chartEl.querySelector('.doughnut-container')) {
      ...
      <div class="doughnut-container">
        <div class="doughnut-labels">
          <h4 style="margin: 0 0 1rem 0; font-size: 0.9rem; color: var(--text-secondary);">${mode === 'power' ? 'Power by Group' : 'Consumption by Group'}</h4>
          <div class="labels-list"></div>
        </div>
        <div class="doughnut-chart"><canvas id="energy-doughnut-canvas"></canvas></div>
      </div>`;
    } else {
    ...
    if (!energyDoughnutChart) {
        energyDoughnutChart = new Chart(canvas.getContext('2d'), {
            type: 'doughnut',
            data: { labels: groupData.map(g => g.name), datasets: [{ data: groupData.map(g => g.value), backgroundColor: groupData.map(g => g.color), borderWidth: 2, borderColor: '#ffffff' }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: (context) => `${context.label || ''}: ${(context.parsed || 0).toFixed(2)} ${unit}` } } } }
        });
    } else {
        ...
        energyDoughnutChart.options.plugins.tooltip.callbacks.label = (context) => `${context.label || ''}: ${(context.parsed || 0).toFixed(2)} ${unit}`;
...
function fetchAndUpdateCurrentConsumption() {
    ...
    const oldChannelGroups = {
        'red-outlets': { channels: [0, 1, 2] }, 'white-outlets': { channels: [3, 4, 5] },
        'ventilation': { channels: [6, 7, 8] }, 'other': { channels: [9, 10, 11] }
    };

    if (currentConsumptionInterval) clearInterval(currentConsumptionInterval);
    ...
    const fetchAllGroups = async () => {
        totalCurrentConsumptionWh = 0;
        await Promise.all(Object.entries(oldChannelGroups).map(([id, group]) => fetchGroupConsumption(id, group)));

        const whiteRaw = groupConsumptionData['white-outlets'] || 0;
        const vent = groupConsumptionData['ventilation'] || 0;
        const whiteAdj = Math.abs(whiteRaw - vent);
        groupConsumptionData['white-outlets'] = whiteAdj;
        totalCurrentConsumptionWh = (groupConsumptionData['red-outlets'] || 0) + whiteAdj + vent + (groupConsumptionData['other'] || 0);
        ...
        if (whiteGroupEl) {
            ...
            if (kwhEl) kwhEl.textContent = `${(whiteAdj / 1000).toFixed(3)} kWh`;
            if (whEl) whEl.textContent = `${whiteAdj} Wh`;
        }
        ...
        Object.entries(channelGroups).forEach(([groupId, group]) => {
            ...
            group.channels.forEach((channel, index) => { chartDataForUpdate[channel] = { value: index === 0 ? consumption : 0 }; });
        });

        const totalEl = el('#energy-total');
        if (totalEl) {
            totalEl.innerHTML = `<div class="total-consumption"><div class="total-label">Total Consumption</div><div class="total-kwh">${(totalCurrentConsumptionWh / 1000).toFixed(3)} kWh</div></div>`;
        }
...
let realtimeCharts = {
    main: null, secondary: null, laiMax: null, humidity: null, light: null,
    occupancy: null, illuminance: null, distance: null, motion: null, rssi: null, snr: null, battery: null
};

let chartData = {
    main: { labels: [], data: [] }, secondary: { labels: [], data: [] }, laiMax: { labels: [], data: [] },
    humidity: { labels: [], data: [] }, light: { labels: [], data: [] }, occupancy: { labels: [], data: [] },
    illuminance: { labels: [], data: [] }, distance: { labels: [], data: [] }, motion: { labels: [], data: [] },
    rssi: { labels: [], data: [] }, snr: { labels: [], data: [] }, battery: { labels: [], data: [] },
...
function updatePowerUsageChart(timestamp, values) {
    ...
    data.labels.push(timestamp); data.red.push(values.red); data.white.push(values.white); data.ventilation.push(values.ventilation);
    if (data.labels.length > MAX_CHART_POINTS) { data.labels.shift(); data.red.shift(); data.white.shift(); data.ventilation.shift(); }
...
function initRealtimeCharts() {
    ...
    const chartConfigs = {
        ...
        'TEMPEX':   { main: { label: 'Temperature', color: '#f59e0b', title: '🌡️ Temperature', unit: '°C' }, secondary: { label: 'Humidity', color: '#3b82f6', title: '💧 Humidity', unit: '%' } },
        'DESK':     { main: { label: 'Occupancy', color: '#10b981', title: '👤 Occupancy', unit: 'Status' }, secondary: { label: 'Temperature', color: '#f59e0b', title: '🌡️ Temperature', unit: '°C' }, humidity: { label: 'Humidity', color: '#10b981', title: '💧 Humidity', unit: '%' } },
        'EYE':      { main: { label: 'Temperature', color: '#f59e0b', title: '🌡️ Temperature', unit: '°C' }, secondary: { label: 'Humidity', color: '#3b82f6', title: '💧 Humidity', unit: '%' }, light: { label: 'Light', color: '#fbbf24', title: '💡 Light Level', unit: 'lux' }, motion: { label: 'Motion', color: '#ec4899', title: '🏃 Motion', unit: 'MotionStatus' }, occupancy: { label: 'Occupancy', color: '#10b981', title: '👤 Occupancy', unit: 'Status' } },
        'OCCUP':    { main: { label: 'Occupancy', color: '#10b981', title: '👤 Occupancy', unit: 'Status' }, illuminance: { label: 'Illuminance', color: '#fbbf24', title: '💡 Illuminance', unit: 'IlluminanceStatus' }, distance: { label: 'Distance', color: '#a855f7', title: '📏 Distance', unit: 'mm' } },
        'PIR_LIGHT':{ main: { label: 'Presence', color: '#10b981', title: '👤 Motion Detection', unit: 'Status' }, secondary: { label: 'Daylight', color: '#fbbf24', title: '☀️ Daylight Level', unit: 'DaylightLevel' } },
        'SON':      { main: { label: 'LAeq', color: '#8b5cf6', title: '🔊 Sound Level', unit: 'dB' }, secondary: { label: 'LAI', color: '#ec4899', title: '📢 Sound Impact', unit: 'dB' }, laiMax: { label: 'LAImax', color: '#9333ea', title: '💥 Max Sound Impact', unit: 'dB' } },
        'COUNT':    { main: { label: 'In', color: '#10b981', title: '📥 Number of Employees In', unit: 'Persons' }, secondary: { label: 'Out', color: '#ef4444', title: '📤 Number of Employees Out', unit: 'Persons' } },
        'ENERGY':   { main: { label: 'Consumption', color: '#f59e0b', title: '⚡ Energy Consumption', unit: 'kWh' }, secondary: { label: 'Consumption by Group', color: '#ef4444', title: '🔌 Consumption by Group', unit: 'kWh' } },
        'CONSO':    { main: { label: 'Consumption', color: '#f59e0b', title: '⚡ Energy Consumption', unit: 'kWh' }, secondary: { label: 'Consumption by Group', color: '#ef4444', title: '🔌 Consumption by Group', unit: 'kWh' } }
    };

    const config = chartConfigs[devType] || { main: { label: 'Metric A', color: '#6366f1', title: '📊 Primary Metric' }, secondary: { label: 'Metric B', color: '#8b5cf6', title: '📈 Secondary Metric' } };
    ...
    if (secondaryTitle && config.secondary) secondaryTitle.textContent = config.secondary.title;
    else console.warn('Secondary chart config or title element not found.');
    ...
    if (mainCtx) realtimeCharts.main = new Chart(mainCtx, createChartConfig(config.main.label, config.main.color, config.main.unit || ''));
    if (secondaryCtx) {
        if (devType === 'ENERGY' || devType === 'CONSO') realtimeCharts.secondary = new Chart(secondaryCtx, createEnergyPowerUsageChartConfig());
        else if (devType === 'OCCUP') { const c = el('#secondary-chart-container'); if (c) c.style.display = 'none'; }
        else realtimeCharts.secondary = new Chart(secondaryCtx, createChartConfig(config.secondary.label, config.secondary.color, config.secondary.unit || ''));
    }
    if (config.laiMax && laiMaxCtx) { const c = el('#laimax-chart-container'); if (c) { c.style.display = 'block'; el('#realtime-chart-laimax-title').textContent = config.laiMax.title; } realtimeCharts.laiMax = new Chart(laiMaxCtx, createChartConfig(config.laiMax.label, config.laiMax.color, config.laiMax.unit || '')); }
    if (config.humidity && humidityCtx) { const c = el('#humidity-chart-container'); if (c) c.style.display = 'block'; realtimeCharts.humidity = new Chart(humidityCtx, createChartConfig(config.humidity.label, config.humidity.color, config.humidity.unit || '')); }
    if (config.light && lightCtx) { const c = el('#light-chart-container'); if (c) c.style.display = 'block'; realtimeCharts.light = new Chart(lightCtx, createChartConfig(config.light.label, config.light.color, config.light.unit || '')); }
    if (config.occupancy && occupancyCtx) { const c = el('#occupancy-chart-container'); if (c) c.style.display = 'block'; realtimeCharts.occupancy = new Chart(occupancyCtx, createChartConfig(config.occupancy.label, config.occupancy.color, config.occupancy.unit || '')); }
    if (config.illuminance && illuminanceCtx) { const c = el('#illuminance-chart-container'); if (c) c.style.display = 'block'; realtimeCharts.illuminance = new Chart(illuminanceCtx, createChartConfig(config.illuminance.label, config.illuminance.color, config.illuminance.unit || '')); }
    if (config.distance && distanceCtx) { const c = el('#distance-chart-container'); if (c) c.style.display = 'block'; realtimeCharts.distance = new Chart(distanceCtx, createChartConfig(config.distance.label, config.distance.color, config.distance.unit || '')); }
    if (config.motion && motionCtx) { const c = el('#motion-chart-container'); if (c) c.style.display = 'block'; realtimeCharts.motion = new Chart(motionCtx, createChartConfig(config.motion.label, config.motion.color, config.motion.unit || '')); }
    if (rssiCtx) realtimeCharts.rssi = new Chart(rssiCtx, createChartConfig('RSSI (dBm)', '#ef4444', 'dBm'));
    if (snrCtx) realtimeCharts.snr = new Chart(snrCtx, createChartConfig('SNR (dB)', '#3b82f6', 'dB'));

    const pauseBtn = el('#chart-pause');
    const clearBtn = el('#chart-clear');
    if (pauseBtn) pauseBtn.addEventListener('click', () => { chartsPaused = !chartsPaused; pauseBtn.textContent = chartsPaused ? '▶️ Resume' : '⏸️ Pause'; });
    if (clearBtn) clearBtn.addEventListener('click', clearAllCharts);
}
...
function createEnergyPowerUsageChartConfig() {
    ...
    return {
        type: 'line',
        data: { labels: [], datasets: [
            { label: 'Red (kW)',        data: [], borderColor: '#ef4444', backgroundColor: 'transparent', fill: false, tension: 0.3, pointRadius: 2, pointHoverRadius: 4, borderWidth: 2 },
            { label: 'White (kW)',      data: [], borderColor: '#64748b', backgroundColor: 'transparent', fill: false, tension: 0.3, pointRadius: 2, pointHoverRadius: 4, borderWidth: 2 },
            { label: 'Ventilation (kW)',data: [], borderColor: '#3b82f6', backgroundColor: 'transparent', fill: false, tension: 0.3, pointRadius: 2, pointHoverRadius: 4, borderWidth: 2 }
        ]},
...
function createChartConfig(label, color, yAxisUnit = '', currentDate) {
    ...
    const hideUnits = ["IlluminanceStatus", "Level", "Status", "LightStatus", "MotionStatus", "DaylightLevel", "Persons"];
    if (yAxisUnit && !hideUnits.includes(yAxisUnit) && !label.includes(`(${yAxisUnit})`)) yAxisLabel += ' (' + yAxisUnit + ')';
    return {
        type: 'line',
        data: { labels: [], datasets: [{ label, data: [], borderColor: color, backgroundColor: 'transparent', fill: false, tension: 0.1, pointRadius: 3, pointHoverRadius: 5, pointBackgroundColor: color, pointBorderColor: color, pointBorderWidth: 2 }] },
...
function getChartOptionsWithUnits(yAxisLabel = '', yAxisUnit = '', currentDate = '') {
    ...
    switch (yAxisUnit) {
        case 'ppm':  yAxisConfig.suggestedMin = 0; yAxisConfig.suggestedMax = 2000; break;
        case 'kW':   yAxisConfig.beginAtZero = true; yAxisConfig.grace = '10%'; break;
        case '°C':   yAxisConfig.suggestedMin = 15; yAxisConfig.suggestedMax = 30; break;
        case '%':    yAxisConfig.min = 0; yAxisConfig.max = 100; break;
        case 'dBm':  yAxisConfig.suggestedMin = -120; yAxisConfig.suggestedMax = -30; break;
        case 'dB':
            if (yAxisLabel.includes('Sound') || yAxisLabel.includes('LAeq') || yAxisLabel.includes('LAI')) { yAxisConfig.suggestedMin = 0; yAxisConfig.suggestedMax = 100; }
            else { yAxisConfig.suggestedMin = -10; yAxisConfig.suggestedMax = 15; }
            break;
        case 'lux':  yAxisConfig.suggestedMin = 0; yAxisConfig.suggestedMax = 1000; break;
        case 'kWh':  yAxisConfig.beginAtZero = true; yAxisConfig.grace = '10%'; break;
        case 'W':    yAxisConfig.beginAtZero = true; yAxisConfig.grace = '10%'; break;
        case 'Status': yAxisConfig.min = 0; yAxisConfig.max = 1; yAxisConfig.ticks = { ...yAxisConfig.ticks, stepSize: 1, callback: (v) => v === 1 ? 'Occupied' : (v === 0 ? 'Free' : '') }; break;
        case 'MotionStatus': yAxisConfig.min = 0; yAxisConfig.beginAtZero = false; yAxisConfig.ticks.stepSize = 1; break;
        case 'DaylightLevel': yAxisConfig.min = 0; yAxisConfig.max = 2; yAxisConfig.ticks = { ...yAxisConfig.ticks, callback: (v) => ({ dim: 'Dim', bright: 'Bright' })[v] ?? '' }; // fall through
        case 'IlluminanceStatus': yAxisConfig.min = 0; yAxisConfig.max = 2; yAxisConfig.ticks = { ...yAxisConfig.ticks, stepSize: 1, callback: (v) => ({ 0: 'Disable', 1: 'Dim', 2: 'Bright' })[v] ?? '' }; break;
        case 's':       yAxisConfig.beginAtZero = true; yAxisConfig.grace = '10%'; break;
        case 'persons': yAxisConfig.beginAtZero = true; yAxisConfig.grace = '10%'; yAxisConfig.ticks.stepSize = 1; break;
        case 'mm':      yAxisConfig.beginAtZero = true; yAxisConfig.grace = '10%'; break;
        default: yAxisConfig.beginAtZero = false;
    }
    return {
        ...
        plugins: {
            ...
            title: { display: true, text: currentDate, position: 'top', align: 'end', font: { size: 12, weight: 'normal' }, color: '#666', padding: { bottom: 10 } },
            tooltip: { callbacks: { label: (context) => { let label = context.dataset.label || ''; if (label) label += ': '; if (context.parsed.y !== null) { label += context.parsed.y; if (yAxisUnit) label += ' ' + yAxisUnit; } return label; } } }
        },
        scales: {
            x: { display: true, title: { display: true, text: 'Time', font: { size: 14, weight: 'bold' } }, grid: { color: 'rgba(0,0,0,0.1)', drawBorder: false }, ticks: { maxTicksLimit: 8, font: { size: 11 } } },
            y: yAxisConfig
        },
        animation: { duration: 0 },
        elements: { point: { radius: 2, hoverRadius: 4 }, line: { tension: 0.3 } }
    };
}

function getChartOptions() { return getChartOptionsWithUnits('', '', ''); }
...
function updateRealtimeCharts(data) {
    ...
    function getBatteryLevel(data) {
        ...
        if (typeof data['vdd (mV)'] === 'number') { const b = vddToBatteryPercent(data['vdd (mV)']); if (b != null) return b; }
        if (typeof data['vdd (v)'] === 'number') { const b = vddToBatteryPercent(Math.round(data['vdd (v)'] * 1000)); if (b != null) return b; }
        return 0;
    }
    ...
    switch (devType) {
        ...
        case 'TEMPEX':
            ...
            break;
        case 'DESK': {
            const temp = data['temperature (°C)'], hum = data['humidity (%)'];
        ...
        case 'OCCUP':
            ...
            if (data.light != null) {
                ...
                const s = (data.light || 'disable').toLowerCase();
                updateChart(realtimeCharts.illuminance, chartData.illuminance, timestamp, s === 'dim' ? 1 : s === 'bright' ? 2 : 0);
            }
            break;
        case 'PIR_LIGHT':
            updateChart(realtimeCharts.main, chartData.main, timestamp, data.presence ? 1 : 0);
        ...
        case 'CONSO': {
            const sumW = (chs) => chs.reduce((s, ch) => s + (Number(channelPowerData[ch]) || 0), 0);
            const redW = Math.abs(sumW([0, 1, 2])), ventW = Math.abs(sumW([6, 7, 8]));
            const whiteW = Math.abs(ventW - sumW([3, 4, 5])), otherW = Math.abs(sumW([9, 10, 11]));
            updateChart(realtimeCharts.main, chartData.main, timestamp, (redW + whiteW + ventW + otherW) / 1000);
            updatePowerUsageChart(timestamp, { red: redW / 1000, white: whiteW / 1000, ventilation: ventW / 1000 });
            break;
        }
    }
}
...
function updateChart(chart, dataStore, timestamp, value) {
    ...
    if (dataStore.labels.length > MAX_CHART_POINTS) { dataStore.labels.shift(); dataStore.data.shift(); }
...
function updateBatteryChart(batteryPct) {
    ...
    if (chartData.battery.labels.length > MAX_CHART_POINTS) { chartData.battery.labels.shift(); chartData.battery.data.shift(); }
...
function initEnergyPlaceholders() {
    ...
    const totalEl = el('#energy-total');
    if (totalEl) totalEl.innerHTML = `<div class="total-consumption"><div class="total-label">Total Power</div><div class="total-kwh">-- kW</div><div class="total-wh">-- W</div></div>`;
}
